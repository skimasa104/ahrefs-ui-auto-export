...
async function main() {
  ...
  if (!cfg.genre) {
    throw new Error("Usage: npm run discover:targets -- <ジャンル>  例: npm run discover:targets -- ED");
  }
  if (!cfg.serperApiKey) {
  ...
  const keywords = buildAffiliateKeywords_(cfg.genre, cfg.maxKeywords);
  console.log(`[info] genre=${cfg.genre} keywords=${keywords.length} source=serper-api`);
...
function buildConfig_() {
  return {
    genre: String(process.argv.slice(2).join(" ") || "").trim(),
    maxKeywords: clampInt_(process.env.DISCOVERY_MAX_KEYWORDS, 15, 1, 50),
...
function buildAffiliateKeywords_(genre, maxCount) {
  const modifiers = [
    "おすすめ",
    "比較",
    "ランキング",
  ...
  const raw = [genre, ...modifiers.map((m) => `${genre} ${m}`)];
  const uniq = [];
  const seen = new Set();
  for (const q of raw) {
    const k = normalizeKey_(q);
